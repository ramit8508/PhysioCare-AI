    ...
    <DashboardLayout role="doctor">
      <div className="p-6 md:p-8 max-w-350 mx-auto space-y-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Create slots, approve bookings, and send prescriptions.</p>
        </div>

        <GlassCard className="p-5 space-y-3">
          <p className="text-sm font-semibold text-foreground">Create Time Slot</p>
          <div className="grid sm:grid-cols-2 gap-3">
            <input
              type="datetime-local"
              value={startAt}
              onChange={(event) => setStartAt(event.target.value)}
              className="rounded-xl bg-secondary/50 border border-white/6 px-3 py-2 text-sm"
            />
            <input
              type="datetime-local"
              value={endAt}
        ...
        <GlassCard className="p-5 space-y-3">
          ...
          {slots.length === 0 ? (
            ...
            <div className="space-y-2">
              {slots.map((slot: any) => (
                <div key={slot.id} className="rounded-xl border border-white/6 bg-secondary/30 p-3">
          ...
          {appointments.length === 0 ? (
            ...
            <div className="space-y-3">
              {appointments.map((appointment: any) => (
                <div key={appointment.id} className="rounded-xl border border-white/6 bg-secondary/30 p-4 space-y-3">
                  ...
                  {appointment.status === "APPROVED" && appointment.meetingUrl && (
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <button
                        ...
                        className="inline-flex px-3 py-1.5 rounded-lg bg-secondary text-foreground text-xs font-semibold"
                  ...
                  {prescriptionForAppointment === appointment.id && (
                    <div className="space-y-2 rounded-xl border border-white/6 p-3">
                      {prescribedExercises.map((exercise, index) => (
                        <div key={`exercise-row-${index}`} className="grid grid-cols-[1fr_140px_110px] gap-2">
                          <input
                            ...
                            className="w-full rounded-lg bg-secondary/50 border border-white/6 px-3 py-2 text-sm"
                          />
                          <input
                            type="number"
                            min={1}
