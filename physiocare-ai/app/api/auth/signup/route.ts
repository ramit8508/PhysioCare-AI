...
import { consumeVerifiedEmailToken, isValidEmail } from "@/lib/email-otp";
...
export async function POST(request: Request) {
  ...
  const verified = consumeVerifiedEmailToken(email, otpVerificationToken);
  if (!verified) {
    return NextResponse.json(
      { error: "Email verification required. Verify OTP and try again." },
      { status: 400 },
    );
  }
